...
const double resting_speed = 500;
...
void on_key(char key, key_event_type_t type, double held_time, state_t *state) {
  ...
  if (type == KEY_PRESSED) {
    switch (key) {
    case RIGHT_ARROW:
      body_set_velocity(state->user,
                        (vector_t){resting_speed + (ACCEL * held_time), 0});
      break;
    case LEFT_ARROW:
      body_set_velocity(
          state->user,
          (vector_t){-1 * (resting_speed + (ACCEL * held_time)), 0});
      break;
    }
...
void reset_game(body_t *body1, body_t *body2, vector_t axis, void *aux,
  ...
  body_set_velocity(body1, BALL_INIT_VEL);
  body_set_centroid(body1, BALL_INIT_POS);
...
bool emscripten_main(state_t *state) {
  ...
  sdl_render_scene(state->scene, state->ball);
  return false;
}
