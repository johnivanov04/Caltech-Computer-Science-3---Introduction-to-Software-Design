...
void test_response_long_body() {
    ...
    const char EXP_RESP_SUFFIX[] = "\r\n\r\n";
    const size_t EXP_RESP_LEN = BODY_LEN + sizeof(EXP_RESP_PREFIX) + sizeof(EXP_RESP_SUFFIX) - 2;
    char *exp_resp = malloc(EXP_RESP_LEN + 1);
    strcpy(exp_resp, EXP_RESP_PREFIX);
    strcat(exp_resp, body);
    strcat(exp_resp, EXP_RESP_SUFFIX);
...
void invalid_status(void *aux) {
    (void) aux;
    char *resp = response_format(7, NULL);
    free(resp);
}
